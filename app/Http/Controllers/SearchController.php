...
class SearchController extends Controller
{
    ...
    public function get_or_make_filters(SearchRequest $request) {
        ...
        if ($count == 0 || ($count == 1 && array_key_exists('search', $params))) {
            ...
            $filters = $this->getCachedFilters($locale);
            if (!$filters) {
                $filters = $make_filters();
                $this->rememberFilters($locale, $filters);
                return $filters;
            } else {
                return $filters;
            }
    ...
    protected function getCachedFilters(string $locale): ?string
    {
        try {
            return cache()->tags(['filters'])->get($locale);
        } catch (BadMethodCallException $exception) {
            return cache()->get($this->filterCacheKey($locale));
        }
    }

    protected function rememberFilters(string $locale, string $filters): void
    {
        try {
            cache()->tags(['filters'])->forever($locale, $filters);
        } catch (BadMethodCallException $exception) {
            cache()->forever($this->filterCacheKey($locale), $filters);
        }
    }

    protected function filterCacheKey(string $locale): string
    {
