...
class ItemController extends Controller
{
    ...
    public function wishlist(Item $item)
    {
        ...
        $this->forgetTaggedCache('wishlist', $item);
    ...
    public function closet(Item $item)
    {
        ...
        $status = $attached ? 'added' : 'removed';
        $this->forgetTaggedCache('closet', $item);

        return back()->withStatus(trans("ui.closet.{$status}", ['item' => Str::limit($item->english_name, 28)]));
    }

    protected function forgetTaggedCache(string $tag, Item $item): void
    {
        try {
            cache()->tags([$tag])->forget($item->getKey());
        } catch (BadMethodCallException $exception) {
            // File cache in local dev does not support tags.
        }
    }
